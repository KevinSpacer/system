	...
	<div class="manage-box">
		...
		<div class="table-content">
			...
			<div class="tool-btn">
				<el-button
					type="primary"
					@click="$navigateTo('/activityTimeLimitAction')"
			...
			>
				...
				<template #btnActive="{ row }">
					...
					<el-button
						size="small"
						type="primary"
						@click="$navigateTo('/activityTimeLimitAction', { viewId: row.id })"
					>
						{{ $LANG_TEXT('编辑') }}
					</el-button>
				</template>
...
const getTableList = () => {
	...
	activeStore.fetfchGetLimitActivitiesPageList(params).then((res) => {
		tableData.value = res
	})
}
...
// 删除接口
const deleteApi = (ids) => {
	activeStore.fetchRemoveLimitActivities(ids).then((res) => {
		ElMessage({
			type: 'success',
