<template>
	...
	<ml-dialog
		ref="formDialogRef"
		class="formDialog"
		:title="$LANG_TEXT(choseItem ? '修改模板' : '添加模板')"
...
const getTableList = () => {
	...
	deviceStore.fetchGetPrinterReceiptTemplatePageList(params).then((res) => {
		tableData.value = res
	})
}
...
const confirmDelete = (item) => {
	deviceStore
		.fetchRemovePrinterReceiptTemplate({ id: item.id })
		.then((res) => {
			ElMessage({
				type: 'success',
...
const formRules = reactive({
	name: [
		{
			message: proxy.$LANG_TEXT('请输入模板名称'),
			required: true,
			trigger: 'change'
		},
		{
...
const dialogConfirm = async (call) => {
	...
	// console.log(detail)
	const api = detail
		? 'fetchEditPrinterReceiptTemplate'
		: 'fetchAddPrinterReceiptTemplate'
