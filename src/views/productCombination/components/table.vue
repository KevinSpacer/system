		...
		<template #content>
			...
			>
				<template #form>
					...
					<el-form-item :label="$LANG_TEXT('所属分类')" prop="classifyId">
						<layz-select
							ref="formLayzSelectRef"
							class="inline-block"
							popperClass="form-select-popper"
							:listFun="getClassifyFun"
...
const getTableList = () => {
	...
	productStore.fetchClassifyCombineGetPageList(params).then((res) => {
		tableData.value = res
	})
}
...
const confirmDelete = (item) => {
	productStore.fetchDeleteClassifyCombine(item.id).then((res) => {
		ElMessage({
			type: 'success',
...
const formRules = reactive({
	...
	classifyId: [
		{
			message: proxy.$LANG_TEXT('请选择所属分类'),
			required: true,
			trigger: 'change'
		}
	]
...
const dialogConfirm = async (call) => {
	...
	const api = detail ? 'fetchClassifyCombineEdit' : 'fetchClassifyCombineAdd'
