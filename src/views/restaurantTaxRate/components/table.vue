<template>
	...
	<ml-dialog
		ref="formDialogRef"
		class="formDialog"
		:title="$LANG_TEXT(choseItem?'修改税率':'添加税率')"
	...
	>
		<template #content>
			...
			>
				<template #form>
					<el-form-item :label="`${$LANG_TEXT('税率名称')}：`" prop="name">
						<second-input
							:placeholder="$LANG_TEXT('税率名称')"
							v-model:firstValue="formData.name"
							v-model:secondValue="formData.nameLanguage"
						></second-input>
					</el-form-item>

					<el-form-item :label="`${$LANG_TEXT('税率值')}：`" prop="value">
						<el-input
							v-model.number="formData.value"
							type="number"
							:placeholder="`${$LANG_TEXT('请输入税率值')}`"
						>
							<template #suffix>
								<div>%</div>
							</template>
...
const getTableList = () => {
	...
	restaurantStore.fetchGetTaxRatePageList(params).then((res) => {
		tableData.value = res;
	});
};
...
const confirmDelete = (item) => {
	restaurantStore.fetchDeleteTaxRate(item.id).then((res) => {
		ElMessage({
			type: "success",
...
const formRules = reactive({
	name: [
		{
			message: proxy.$LANG_TEXT("请输入税率名称"),
			required: true,
			trigger: "change",
		},
		{
	...
	value: [
		{
			message: proxy.$LANG_TEXT("请输入税率值"),
			required: true,
			trigger: "change",
		},
	]
...
const dialogConfirm = async (call) => {
	...
	const api = detail?'fetchTaxRateEdit':'fetchTaxRateAdd'
