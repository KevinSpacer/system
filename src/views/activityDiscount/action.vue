  ...
  dishes: [
    {
      required: true,
      validator: (rules, value, call) => {
        const active = productData.value[0];
        // console.log(active)

        // 已选择商品
        if (active.dishesIds.length) {
          const isEmptys = ["", null, undefined];
          const productList = active.productList;
          for (let i = 0; i < productList.length; i++) {
            const item = productList[i];
            const price = item.price;

            // 判空
            if (isEmptys.includes(price)) {
              call(new Error(proxy.$LANG_TEXT("请填写好菜品价格")));
              return;
            }
          }
...
const submit = async (call) => {
  // 赋值
  formData.dishes = productData.value[0].productList.map((item) => {
    return {
      goodsId: item.id,
      type: "DISHES",
      price: item.price,
  ...
  const api = viewId ? "fetchEditSpecialOffer" : "fetchAddSpecialOffer";
...
const getCurrProductInfo = async () => {
  try {
    const res = await activeStore.fetchGetSpecialOfferDetails(viewId);
