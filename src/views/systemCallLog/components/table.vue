		...
		<div class="table-content">
			...
			>
				<template #userType="{ row }">
					<div>
						{{ $LANG_TEXT(userTypes[row.userType]) }}
					</div>
				</template>
				<template #type="{ row }">
					<div>
						{{ $LANG_TEXT(callTypes.find((item) => item.value == row.type).label) }}
					</div>
				</template>
			</ml-table>
		</div>
...
import { callTypeOpts } from '@/utils/options'

const callTypes = callTypeOpts()
...
const getTableList = () => {
	...
	systemStore.fetchGetPunchCardRecordSelectPageList(params).then((res) => {
		tableData.value = res
	})
}
